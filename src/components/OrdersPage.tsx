...
function OrderItemsEditor({ items, setItems }: { items: OrderItem[]; setItems: (items: OrderItem[]) => void }) {
  ...
  return <div className="space-y-3"><div className="flex items-center justify-between"><p className="text-sm font-semibold text-slate-700">المنتجات المطلوبة</p><button type="button" onClick={addItem} className="flex items-center gap-1.5 text-indigo-600 text-sm font-medium"><Plus className="w-4 h-4" />إضافة منتج</button></div>{items.map((item, index) => <div key={index} className="bg-slate-50 rounded-xl p-3 space-y-2"><div className="flex items-center justify-between"><span className="text-xs font-medium text-slate-500">منتج {index + 1}</span>{items.length > 1 && <button type="button" onClick={() => removeItem(index)} className="p-1 hover:bg-red-100 rounded-lg text-slate-400 hover:text-red-500"><X className="w-3.5 h-3.5" /></button>}</div><div className="grid grid-cols-3 gap-2"><input className="form-input col-span-3 sm:col-span-1" placeholder="اسم المنتج *" value={item.productName} onChange={(event) => updateItem(index, "productName", event.target.value)} /><input className="form-input text-center" type="number" min="1" value={item.quantity} onChange={(event) => updateItem(index, "quantity", Number(event.target.value))} /><input className="form-input text-center" type="number" min="0" step="0.01" value={item.unitPrice || ""} onChange={(event) => updateItem(index, "unitPrice", Number(event.target.value))} /></div><input className="form-input text-sm" placeholder="ملاحظات (اختياري)" value={item.notes ?? ""} onChange={(event) => updateItem(index, "notes", event.target.value)} /><div className="text-left text-sm font-bold text-indigo-600">{money(item.quantity * item.unitPrice)}</div></div>)}</div>;
}
