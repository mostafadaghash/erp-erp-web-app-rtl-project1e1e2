...
const statusClass = (status: string) => {
  ...
  if (status === "pending" || status === "shipped") return "badge-info";
  if (status === "returned") return "badge-warning";
  return "badge-danger";
};

type Modal = "create" | "ship" | "deliver" | "return" | "cancel" | "reverse-confirmation" | "settle" | "reverse-settlement" | "details" | null;
type Selected = {
  _id: Id<"deliveries">;
...
const successMessage = (modal: Exclude<Modal, "details" | null>) => ({
...
export function DeliveriesPage({ createRequestToken }: { createRequestToken?: number }) {
  ...
  const operationRequestId = useRef(makeRequestId());
  ...
  const deliveryDetails = useQuery(api.deliveries.get, modal === "details" && selected ? { id: selected._id } : "skip");
  ...
  const gross = useMemo(() => unsettled?.filter(item => checked.has(String(item._id))).reduce((sum, item) => sum + (item.codAmount ?? 0), 0) ?? 0, [unsettled, checked]);
  ...
  const validationReason = (() => {
    ...
    if (!selected && modal !== "settle" && modal !== "reverse-settlement") return "اختر الشحنة";
    if (modal === "deliver") {
  ...
  const metricCards = [
    { label: "قيد التحصيل لدى شركات الشحن", value: stats?.codWithCarriers, icon: Truck, tone: "bg-blue-50 text-blue-700" },
    { label: "تمت تسويته", value: stats?.codSettled, icon: CheckCircle2, tone: "bg-emerald-50 text-emerald-700" },
    { label: "أُلغي تحصيله", value: stats?.codReversed, icon: RotateCcw, tone: "bg-amber-50 text-amber-700" },
    { label: "رسوم شركات الشحن", value: stats?.carrierFees, icon: CircleDollarSign, tone: "bg-rose-50 text-rose-700" },
  ];
    ...
    <div dir="rtl" className="space-y-6 p-4 lg:p-6" data-testid="deliveries-page">
      <header className="erp-page-header">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {branches && <select data-testid="delivery-branch-select" className="form-input min-w-52" value={activeBranch ?? ""} onChange={event => handleBranchChange(event.target.value)}><option value="">اختر الفرع</option>{branches.filter(branch => branch.isActive).map(branch => <option key={branch._id} value={branch._id}>{branch.name}</option>)}</select>}
          {canCreate && <button data-testid="delivery-create-open" className="btn-primary" onClick={() => open("create")} disabled={!activeBranch}>شحنة جديدة</button>}
        </div>
      </header>
      ...
      <section className="erp-section">
        <div className="erp-section-header"><div><h2 className="erp-section-title">الشحنات</h2><p className="mt-1 text-xs text-slate-400">متابعة حالة كل شحنة والمبلغ المطلوب تحصيله</p></div></div>
        <div className="overflow-x-auto">
          <table className="data-table min-w-[820px]">
            <thead><tr><th>رقم الشحنة</th><th>العميل</th><th>الحالة</th><th>المبلغ المطلوب تحصيله</th><th>الإجراءات</th></tr></thead>
            <tbody>
              ...
              {rows.map(delivery => <tr key={delivery._id} data-testid="delivery-row" data-delivery-number={delivery.deliveryNumber} data-customer-name={delivery.customerName} data-status={delivery.status}>
                ...
                <td><div className="erp-actions">
                  ...
                  {canEdit && delivery.status === "shipped" && <button className="erp-action" onClick={() => open("return", delivery)}><RotateCcw size={15} /> تسجيل مرتجع</button>}
                  {canConfirm && delivery.status === "shipped" && <button data-testid="delivery-confirm-open" className="erp-action erp-action-primary" onClick={() => open("deliver", delivery)}><PackageCheck size={15} /> تسجيل التسليم</button>}
                  {canReverse && delivery.status === "delivered" && <button className="erp-action erp-action-danger" onClick={() => open("reverse-confirmation", delivery)}><RotateCcw size={15} /> إلغاء تسجيل التسليم</button>}
                  {canPrint && <button className="erp-action" onClick={() => void printDelivery(delivery)}><Printer size={15} /> طباعة</button>}
                </div></td>
              </tr>)}
            </tbody>
          </table>
        </div>
        {(deliveries.status === "CanLoadMore" || deliveries.status === "LoadingMore") && <div className="border-t border-slate-100 p-3 text-center"><button className="erp-action" disabled={deliveries.status === "LoadingMore"} onClick={() => deliveries.loadMore(20)}>{deliveries.status === "LoadingMore" ? "جارٍ تحميل المزيد…" : "عرض المزيد"}</button></div>}
      </section>

      {canViewSettlements && <section className="erp-section">
        <div className="erp-section-header"><div><h2 className="erp-section-title">تسويات مبالغ التحصيل</h2><p className="mt-1 text-xs text-slate-400">نقل المبالغ المحصلة من حساب شركة الشحن إلى الخزينة أو البنك</p></div>{canSettle && <button data-testid="delivery-settlement-open" className="btn-primary" onClick={() => open("settle")} disabled={!activeBranch}>تسوية مبالغ التحصيل</button>}</div>
        <div className="divide-y divide-slate-100">
          {activeBranch && settlements.status === "LoadingFirstPage" && <p role="status" className="p-5 text-slate-500">جارٍ تحميل التسويات…</p>}
      ...
      {modal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><div data-testid="delivery-action-modal" data-modal={modal} className="w-[min(96vw,720px)] max-h-[90vh] space-y-4 overflow-y-auto rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
        ...
        {modal !== "details" && <div className="flex justify-end gap-2 border-t border-slate-100 pt-4"><button className="erp-action" onClick={close} disabled={busy}>إلغاء</button><button data-testid="delivery-action-submit" disabled={busy || Boolean(validationReason)} aria-describedby={validationReason ? "delivery-action-validation" : undefined} title={validationReason ?? undefined} className="btn-primary" onClick={() => {
          ...
          void run(async () => {
            ...
            if (operation === "deliver" && selected) return confirmDelivered({ deliveryId: selected._id, codClearingAccountId: accountId ? accountId as Id<"financialAccounts"> : undefined, date, requestId: operationRequestId.current });
            if ((operation === "return" || operation === "cancel") && selected) return updateStatus({ id: selected._id, status: operation === "return" ? "returned" : "cancelled", reason });
            if (operation === "reverse-confirmation" && selected) return reverseConfirmation({ deliveryId: selected._id, reason, date, requestId: operationRequestId.current });
            if (operation === "settle" && activeBranch) return createSettlement({ deliveryIds: [...checked] as Id<"deliveries">[], sourceAccountId: accountId as Id<"financialAccounts">, destinationAccountId: destinationId as Id<"financialAccounts">, feeAmount, date, branchId: activeBranch, requestId: operationRequestId.current });
            if (operation === "reverse-settlement" && selectedSettlementId) return reverseSettlement({ settlementId: selectedSettlementId, reason: reversalReason.trim(), date, requestId: operationRequestId.current });
