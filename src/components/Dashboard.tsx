...
export function Dashboard({ onOpenReport, permissions }: DashboardProps) {
  ...
  return <div className="erp-dashboard-compact" data-refreshed-at={refreshedAt}>
    ...
    {canViewReports && report === undefined && rangeIsValid && <div className="erp-dashboard-card-grid" aria-label="جارٍ تحميل المؤشرات">{Array.from({ length: 8 }, (_, index) => <div key={index} className="erp-dashboard-card-skeleton animate-pulse" />)}</div>}
