...
export function Dashboard({ onOpenReport, permissions }: DashboardProps) {
  ...
  const cards: DashboardCard[] = report ? [
    ...
    { key: "inventory", title: "قيمة المخزون", value: report.currentBalances.inventoryValue, note: canViewProducts ? `${formatAmount(lowStockProducts?.length ?? 0)} صنف تحت حد الطلب` : "التنبيهات حسب صلاحية المخزون", report: "inventory", icon: Boxes, tone: "slate", protected: !canViewProfits || !canViewProducts },
  ] : [];
  return <div className="erp-dashboard-compact" data-refreshed-at={refreshedAt} data-testid="executive-dashboard">
    ...
    {cards.length > 0 && <section className="erp-dashboard-card-grid" aria-label="المؤشرات التنفيذية الرئيسية">{cards.map((card) => { const Icon = card.icon; const change = compare ? comparisonPercent(card.value, card.comparisonValue) : null; const ChangeIcon = change !== null && change < 0 ? ArrowDownLeft : ArrowUpLeft; const displayValue = card.protected || card.value == null ? "غير متاح" : formatCurrency(card.value); const reportHint = canViewReports ? "فتح التقرير التفصيلي" : "التقرير التفصيلي غير متاح حسب الصلاحية"; return <button key={card.key} type="button" data-testid={`dashboard-card-${card.key}`} className={`erp-dashboard-card tone-${card.tone}`} onClick={() => { if (canViewReports) onOpenReport(card.report); }} disabled={!canViewReports} aria-label={`${card.title}: ${displayValue}. ${reportHint}`}><div className="erp-dashboard-card-head"><div className="min-w-0 text-right"><p className="erp-metric-label">{card.title}</p><p className="erp-dashboard-card-value">{displayValue}</p></div><span className="erp-dashboard-card-icon"><Icon /></span></div><p className="erp-dashboard-card-note">{card.note}</p><div className={`erp-dashboard-card-status ${change === null ? "neutral" : change < 0 ? "down" : "up"}`}>{change !== null ? <><span className="erp-dashboard-status-label"><ChangeIcon />{change < 0 ? "أقل من الفترة السابقة" : "أعلى من الفترة السابقة"}</span><strong>{formatAmount(Math.abs(change))}٪</strong></> : <><span className="erp-dashboard-status-label"><PackageSearch />{reportHint}</span>{canViewReports && <ArrowUpLeft className="erp-dashboard-status-arrow" aria-hidden="true" />}</>}</div></button>; })}</section>}
  </div>;
}
