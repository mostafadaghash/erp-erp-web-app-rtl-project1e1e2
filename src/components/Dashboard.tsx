...
export function Dashboard({ onOpenReport, permissions }: DashboardProps) {
  const canViewExecutiveDashboard = permissions.includes("view_executive_dashboard"); const canViewReports = permissions.includes("view_reports"); const canViewExecutiveData = canViewExecutiveDashboard && canViewReports; const canViewProfits = permissions.includes("view_profits"); const canViewProducts = permissions.includes("view_products");
  ...
  const me = useQuery(api.employees.me); const branches = useQuery(api.reporting.availableBranches, canViewExecutiveData ? {} : "skip") ?? []; const canSelectBranch = me?.role === "admin" || me?.role === "accountant"; const rangeIsValid = validRange(from, to);
  const reportArgs = rangeIsValid ? { from, to, branchId: canSelectBranch && branchId ? branchId : undefined } : null;
  const report = useQuery(api.reporting.overview, canViewExecutiveData && reportArgs ? reportArgs : "skip") as ReportingOverview | undefined;
  const comparisonRange = rangeIsValid ? previousRange(from, to) : null;
  const previousReport = useQuery(api.reporting.overview, canViewExecutiveData && compare && comparisonRange ? { ...comparisonRange, branchId: canSelectBranch && branchId ? branchId : undefined } : "skip") as ReportingOverview | undefined;
  ...
  return <div className="erp-dashboard-compact" data-refreshed-at={refreshedAt}>
    <section className="erp-dashboard-controls" aria-label="فلاتر لوحة التحكم"><div className="erp-dashboard-title-block"><span className="erp-dashboard-title-icon" aria-hidden="true"><LayoutDashboard /></span><div className="min-w-0"><p className="erp-dashboard-breadcrumb">لوحة التحكم</p><h1 className="erp-page-title">لوحة التحكم</h1><p className="erp-dashboard-subtitle">ملخص أداء النشاط</p></div></div><div className="erp-dashboard-filter-grid">
    ...
    {!canViewExecutiveData && <div className="erp-empty-state mt-4">لا تملك صلاحية عرض لوحة التحكم التنفيذية.</div>}
    {canViewExecutiveData && report === undefined && rangeIsValid && <div className="erp-dashboard-card-grid" aria-label="جارٍ تحميل المؤشرات">{Array.from({ length: 8 }, (_, index) => <div key={index} className="erp-dashboard-card-skeleton animate-pulse" />)}</div>}
    {cards.length > 0 && <section className="erp-dashboard-card-grid" aria-label="المؤشرات الرئيسية">{cards.map((card) => { const Icon = card.icon; const change = compare ? comparisonPercent(card.value, card.comparisonValue) : null; const ChangeIcon = change !== null && change < 0 ? ArrowDownLeft : ArrowUpLeft; const displayValue = card.protected || card.value == null ? "غير متاح" : formatCurrency(card.value); return <button key={card.key} type="button" data-testid={`dashboard-card-${card.key}`} className={`erp-dashboard-card tone-${card.tone}`} onClick={() => onOpenReport(card.report)} aria-label={`${card.title}: ${displayValue}. فتح التقرير التفصيلي`}><div className="erp-dashboard-card-head"><div className="min-w-0 text-right"><p className="erp-metric-label">{card.title}</p><p className="erp-dashboard-card-value">{displayValue}</p></div><span className="erp-dashboard-card-icon"><Icon /></span></div><p className="erp-dashboard-card-note">{card.note}</p><div className={`erp-dashboard-card-status ${change === null ? "neutral" : change < 0 ? "down" : "up"}`}>{change !== null ? <><span className="erp-dashboard-status-label"><ChangeIcon />{change < 0 ? "أقل من الفترة السابقة" : "أعلى من الفترة السابقة"}</span><strong>{formatAmount(Math.abs(change))}٪</strong></> : <><span className="erp-dashboard-status-label"><PackageSearch />فتح التقرير التفصيلي</span><ArrowUpLeft className="erp-dashboard-status-arrow" aria-hidden="true" /></>}</div></button>; })}</section>}
  </div>;
}
