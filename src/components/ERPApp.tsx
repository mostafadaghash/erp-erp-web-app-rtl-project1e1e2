...
export type Page =
  ...
  | "reports" | "settings" | "audit-logs" | "treasury" | "supplier-payments"
  | "purchase-returns" | "customer-ledger" | "general-ledger" | "data-export";

const PAGE_PERMISSIONS: Partial<Record<Page, Permission>> = {
  products: "view_products", customers: "view_customers", invoices: "view_invoices",
  "sales-returns": "view_sales_returns", "new-invoice": "create_invoices",
  repairs: "view_repairs", expenses: "view_expenses", suppliers: "view_suppliers",
  orders: "view_orders", deliveries: "view_deliveries", shipments: "view_shipments",
  branches: "view_branches", employees: "view_employees", crm: "view_leads",
  reports: "view_reports", settings: "manage_settings", "audit-logs": "view_audit_logs",
  treasury: "view_finance", "supplier-payments": "view_supplier_ledger",
  "purchase-returns": "view_purchase_returns", "customer-ledger": "view_customer_ledger",
  "general-ledger": "view_general_ledger", "data-export": "export_data",
};

const PAGE_MODULES: Partial<Record<Page, string>> = {
  invoices: "invoices", "sales-returns": "invoices", "new-invoice": "invoices",
  orders: "orders", deliveries: "deliveries", repairs: "repairs", expenses: "expenses",
  suppliers: "suppliers", shipments: "shipments", branches: "branches",
  employees: "employees", crm: "crm", reports: "reports",
};

const PAGE_META: Record<Page, { group: string; title: string }> = {
  dashboard: { group: "الرئيسية", title: "لوحة التحكم" },
  products: { group: "المخزون", title: "الأصناف" },
  ...
  deliveries: { group: "الشحن", title: "عمليات الشحن" },
  ...
  "general-ledger": { group: "الحسابات", title: "الأستاذ العام" },
  "data-export": { group: "الإدارة", title: "تصدير البيانات" },
};

type CreateTarget = "new-invoice" | "orders" | "customers" | "products" | "shipments" | "deliveries" | "expenses" | "repairs";
const CREATE_ACTIONS: Array<{ page: CreateTarget; label: string; permission: Permission }> = [
  ...
  { page: "customers", label: "عميل", permission: "create_customers" },
  { page: "products", label: "صنف", permission: "create_products" },
  { page: "shipments", label: "عملية شراء", permission: "create_shipments" },
  { page: "deliveries", label: "عملية شحن", permission: "create_deliveries" },
  { page: "expenses", label: "مصروف", permission: "create_expenses" },
...
export function ERPApp() {
  ...
  const [customerLedgerTarget, setCustomerLedgerTarget] = useState<{ customerId: Id<"customers">; branchId: Id<"branches"> } | null>(null);
  ...
  const canSelectWorkingBranch = settings !== undefined && me?.role === "admin" && can("manage_branches") && modules.branches !== false;
  ...
  const openCustomerLedger = (customerId: Id<"customers">, branchId: Id<"branches">) => {
  ...
  if (me === undefined) {
    return <div className="flex h-screen items-center justify-center bg-slate-50" dir="rtl"><p className="text-slate-500">جاري تحميل صلاحيات الحساب...</p></div>;
  }

  const authorized = canAccessPage(currentPage);
  const pageMeta = PAGE_META[currentPage];
  const createActions = CREATE_ACTIONS.filter(action => can(action.permission) && isModuleEnabled(action.page));
  const createToken = (page: Page) => createRequest?.page === page ? createRequest.token : undefined;

  return (
    <PermissionProvider permissions={permissions}>
      <div className="flex h-screen overflow-hidden bg-slate-50" dir="rtl">
        {sidebarOpen && <div className="fixed inset-0 z-20 bg-slate-950/50 backdrop-blur-sm lg:hidden" onClick={() => setSidebarOpen(false)} />}
        <div className={`fixed inset-y-0 right-0 z-30 w-72 transform transition-transform duration-300 lg:static ${sidebarOpen ? "translate-x-0" : "translate-x-full lg:translate-x-0"}`}>
          <Sidebar currentPage={currentPage} onNavigate={navigate} brand={brand} modules={modules} permissions={permissions} userName={me.name} role={me.role} onClose={() => setSidebarOpen(false)} />
        </div>

        <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
          <header className="erp-topbar flex h-[72px] flex-shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 lg:px-7">
            <div className="flex min-w-0 items-center gap-3">
              <button className="rounded-xl p-2.5 text-slate-600 transition hover:bg-slate-100 lg:hidden" onClick={() => setSidebarOpen(true)} aria-label="فتح القائمة الرئيسية"><Menu className="h-5 w-5" /></button>
            ...
            <div className="flex items-center gap-2.5">
              {canSelectWorkingBranch && (
                <select data-testid="working-branch-select" className={`hidden rounded-xl border bg-white px-3 py-2 text-xs md:block ${me.branchId ? "border-slate-200 text-slate-700" : "border-amber-300 text-amber-700"}`} value={me.branchId ?? ""} onChange={event => void handleWorkingBranchChange(event.target.value)} aria-label="فرع العمل الحالي">
              ...
              {createActions.length > 0 && (
                <div className="relative">
                  <button type="button" data-testid="quick-action-menu" onClick={() => setQuickMenuOpen(value => !value)} className="btn-primary flex items-center gap-2" aria-expanded={quickMenuOpen}>
                    <Plus className="h-4 w-4" /><span className="hidden sm:inline">إجراء جديد</span><ChevronDown className="h-3.5 w-3.5" />
                  </button>
                  {quickMenuOpen && (
                    <div className="absolute left-0 top-[calc(100%+10px)] z-50 w-52 overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-2xl">
                      {createActions.map(action => (
                        <button key={action.page} type="button" data-testid={`quick-action-${action.page}`} onClick={() => requestCreate(action.page)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2.5 text-right text-sm font-bold text-slate-700 transition hover:bg-slate-50 hover:text-[var(--brand-primary)]"><Plus className="h-3.5 w-3.5" />{action.label}</button>
                      ))}
                    </div>
                  )}
                </div>
              )}
              <div className="flex h-9 w-9 items-center justify-center rounded-xl text-xs font-black text-white shadow-sm" style={{ background: `linear-gradient(135deg, ${brand.primaryColor}, ${brand.secondaryColor})` }} title={me.name}>{me.name.trim().charAt(0) || "م"}</div>
            </div>
          </header>

          <main className="flex-1 overflow-y-auto">
            {!authorized && <div className="flex min-h-[70vh] items-center justify-center p-6"><div className="text-center"><ShieldX className="mx-auto mb-4 h-14 w-14 text-red-400" /><h2 className="text-xl font-bold text-slate-800">غير مصرح بالوصول</h2><p className="mt-2 text-sm text-slate-500">لا يملك حسابك الصلاحية المطلوبة لفتح هذه الصفحة.</p></div></div>}
