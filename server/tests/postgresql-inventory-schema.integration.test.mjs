...
const EXPECTED_COLUMNS = {
  serial_numbers: [
    ...
    ["current_warehouse_id", "uuid", false],
    ["status", "text", true],
  ...
  inventory_movements: [
    ...
    ["movement_type", "text", true],
    ["source_type", "text", true],
    ["source_id", "uuid", true],
    ["posting_batch_id", "uuid", true],
  ...
  inventory_movement_lines: [
    ["id", "uuid", true],
    ["movement_id", "uuid", true],
    ["variant_id", "uuid", true],
    ["quantity_signed", "numeric(18,6)", true],
    ["unit_cost", "numeric(18,4)", true],
    ["total_cost", "numeric(18,4)", true],
  ],
  inventory_line_serials: [
    ["movement_line_id", "uuid", true],
    ["serial_id", "uuid", true],
  ],
  inventory_line_batches: [
    ["movement_line_id", "uuid", true],
  ...
  inventory_stock_positions: [
    ["warehouse_id", "uuid", true],
    ["variant_id", "uuid", true],
    ["on_hand", "numeric(18,6)", true],
    ["reserved", "numeric(18,6)", true],
  ...
  variant_warehouse_cost_projection: [
    ["warehouse_id", "uuid", true],
    ["variant_id", "uuid", true],
    ["weighted_average_cost", "numeric(18,4)", true],
    ["last_purchase_cost", "numeric(18,4)", true],
    ["inventory_value", "numeric(18,4)", true],
    ["updated_at", "timestamp with time zone", true],
  ],
  batch_stock_positions: [
    ["warehouse_id", "uuid", true],
  ...
  stock_reservations: [
    ["id", "uuid", true],
    ["sales_order_id", "uuid", true],
    ["sales_order_line_id", "uuid", true],
    ["warehouse_id", "uuid", true],
    ["variant_id", "uuid", true],
    ["quantity", "numeric(18,6)", true],
    ["status", "text", true],
  ...
  stock_transfers: [
    ...
    ["issuing_branch_id", "uuid", true],
    ["from_warehouse_id", "uuid", true],
    ["to_warehouse_id", "uuid", true],
    ["status", "text", true],
    ["notes", "text", false],
    ["created_by", "uuid", true],
  ...
  stocktake_lines: [
    ...
    ["variant_id", "uuid", true],
    ["book_quantity_at_count", "numeric(18,6)", true],
    ["counted_quantity", "numeric(18,6)", true],
    ["counted_at", "timestamp with time zone", true],
    ["stock_position_version_at_count", "integer", true],
    ["difference", "numeric(18,6)", true],
    ["notes", "text", false],
  ],
  stocktake_line_serials: [
    ["stocktake_line_id", "uuid", true],
    ["serial_id", "uuid", true],
  ],
  stocktake_line_batches: [
    ["stocktake_line_id", "uuid", true],
  ...
  inventory_adjustments: [
    ...
    ["document_number", "bigint", true],
    ["warehouse_id", "uuid", true],
    ["source_stocktake_id", "uuid", false],
  ...
  inventory_adjustment_lines: [
    ...
    ["variant_id", "uuid", true],
    ["quantity_difference", "numeric(18,6)", true],
    ["unit_cost", "numeric(18,4)", true],
  ],
  inventory_adjustment_line_serials: [
    ["adjustment_line_id", "uuid", true],
    ["serial_id", "uuid", true],
  ],
  inventory_adjustment_line_batches: [
    ["adjustment_line_id", "uuid", true],
...
test("03.D creates only the canonical Inventory schema shape", async (t) => {
  if (!databaseUrl) return t.skip("ERP_TEST_DATABASE_URL is not configured");

  await cleanup();
  try {
    const first = await runMigrations({ databaseUrl });
    assert.deepEqual(first.applied, ["0001", "0002", "0003", "0004", "0005"]);
    ...
    await withClient(async (client) => {
      ...
      assert.deepEqual(
        ...
        "03.D must not create duplicate aliases or later-domain relations",
      );
      ...
      const actualByTable = Object.fromEntries(INVENTORY_TABLES.map((table) => [table, []]));
      ...
      const generatedDeferred = await client.query(
        `SELECT
           ...
           ) AS inventory_available_exists,
           EXISTS (
             SELECT 1 FROM information_schema.columns
             WHERE table_schema = 'public'
               AND table_name = 'batch_stock_positions'
               AND column_name = 'available'
           ) AS batch_available_exists`,
      );
      assert.equal(
        generatedDeferred.rows[0].inventory_available_exists,
        false,
        "inventory_stock_positions.available must be added as generated in 03.06, not as writable 03.D data",
      );
      assert.equal(
        generatedDeferred.rows[0].batch_available_exists,
        false,
        "batch_stock_positions.available must be added as generated in 03.06, not as writable 03.D data",
      );
      ...
      const adjustmentLineIdentity = await client.query(
        `SELECT
           EXISTS (
             SELECT 1 FROM information_schema.columns
             WHERE table_schema = 'public'
               AND table_name = 'inventory_adjustment_lines'
               AND column_name = 'id'
           ) AS line_has_id,
           EXISTS (
             SELECT 1 FROM information_schema.columns
             WHERE table_schema = 'public'
               AND table_name = 'inventory_adjustment_line_serials'
               AND column_name = 'adjustment_line_id'
           ) AS serial_child_uses_line_id,
           EXISTS (
             SELECT 1 FROM information_schema.columns
             WHERE table_schema = 'public'
               AND table_name = 'inventory_adjustment_line_batches'
               AND column_name = 'adjustment_line_id'
           ) AS batch_child_uses_line_id`,
      );
      ...
      const futureDomain = await client.query(
        "SELECT to_regclass('public.sales_quotes') IS NULL AS absent",
      );
      ...
      assert.equal(history.rows[4].version, "0005");
      assert.equal(history.rows[4].name, "inventory");
      assert.match(history.rows[4].checksum, /^[0-9a-f]{64}$/);
    });

    const second = await runMigrations({ databaseUrl });
    assert.deepEqual(second.applied, []);
    assert.deepEqual(second.skipped, ["0001", "0002", "0003", "0004", "0005"]);

    const verification = await runMigrations({ databaseUrl, verifyOnly: true });
    assert.deepEqual(verification.applied, []);
    assert.deepEqual(verification.skipped, ["0001", "0002", "0003", "0004", "0005"]);
  } finally {
    await cleanup();
  }
});
