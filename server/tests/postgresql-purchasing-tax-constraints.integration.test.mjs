...
test("03.06 Purchasing / Tax constraints enforce canonical integrity on PostgreSQL 17", async (t) => {
  ...
  try {
    ...
    await withClient(async(client)=>{
      ...
      const latest=history.rows.at(-1); assert.equal(latest.version,"0023"); assert.equal(latest.name,"counterparty_ledger_immutability"); assert.match(latest.checksum,/^[0-9a-f]{64}$/);
