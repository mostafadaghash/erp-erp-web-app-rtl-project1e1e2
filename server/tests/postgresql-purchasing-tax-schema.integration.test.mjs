...
async function seedViewFixture(client) {
  const ids = {
    company: "51000000-0000-4000-8000-000000000001", branch: "51000000-0000-4000-8000-000000000002",
    warehouse: "51000000-0000-4000-8000-000000000003", role: "51000000-0000-4000-8000-000000000004",
    user: "51000000-0000-4000-8000-000000000005", counterparty: "51000000-0000-4000-8000-000000000006",
    category: "51000000-0000-4000-8000-000000000007", unit: "51000000-0000-4000-8000-000000000008",
    product: "51000000-0000-4000-8000-000000000009", productUnit: "51000000-0000-4000-8000-000000000010",
    variant: "51000000-0000-4000-8000-000000000011", invoice: "51000000-0000-4000-8000-000000000012",
    line: "51000000-0000-4000-8000-000000000013", activeReturn: "51000000-0000-4000-8000-000000000014",
    deletedReturn: "51000000-0000-4000-8000-000000000015",
  };
  await client.query(`INSERT INTO companies (id,name,base_currency_code,default_language,timezone,is_active,created_at,updated_at) VALUES ($1,'Purchasing Schema Co','EGP','ar-EG','Africa/Cairo',true,now(),now())`, [ids.company]);
  ...
  await client.query(`INSERT INTO warehouses (id,branch_id,name,code,is_active,created_at,updated_at) VALUES ($1,$2,'Main WH','MAIN-WH',true,now(),now())`, [ids.warehouse, ids.branch]);
  await client.query(`INSERT INTO roles (id,role_key,display_name_key,is_system) VALUES ($1,'PURCH_SCHEMA','roles.purchSchema',true)`, [ids.role]);
  await client.query(`INSERT INTO users (id,name,username,email,password_hash,role_id,default_branch_id,branch_scope_mode,preferred_language,is_active,created_at,updated_at) VALUES ($1,'Purchasing Schema User','purch-schema',NULL,'hash',$2,$3,'ALL','ar-EG',true,now(),now())`, [ids.user, ids.role, ids.branch]);
  await client.query(`INSERT INTO counterparties (id,name,phone,normalized_phone,address,notes,is_active,created_at,updated_at) VALUES ($1,'Schema Supplier',NULL,NULL,NULL,NULL,true,now(),now())`, [ids.counterparty]);
  ...
  await client.query(`INSERT INTO products (id,name,category_id,product_type,base_unit_id,tracking_serial,tracking_batch,tracking_expiry,is_active,created_at,updated_at) VALUES ($1,'Schema Product',$2,'STOCK',$3,false,false,false,true,now(),now())`, [ids.product, ids.category, ids.productUnit]);
  ...
  await client.query(`INSERT INTO product_variants (id,product_id,name,sku,is_default,combination_signature,minimum_selling_price,is_active,created_at,updated_at) VALUES ($1,$2,'Default','PUR-SCHEMA',true,'DEFAULT-SCHEMA',0.0000,true,now(),now())`, [ids.variant, ids.product]);
  await client.query("COMMIT");
  await client.query(`INSERT INTO purchase_invoices (id,branch_id,document_number,document_date,document_version,counterparty_id,warehouse_id,subtotal,discount_total,additional_cost,tax_total,grand_total,paid_total,due_total,payment_status,notes,posted_at,created_by,deleted_at,deleted_by,delete_reason) VALUES ($1,$2,1,CURRENT_DATE,1,$3,$4,400.0000,0.0000,0.0000,0.0000,400.0000,400.0000,0.0000,'POSTED',NULL,now(),$5,NULL,NULL,NULL)`, [ids.invoice, ids.branch, ids.counterparty, ids.warehouse, ids.user]);
  await client.query(`INSERT INTO purchase_invoice_lines (id,purchase_invoice_id,variant_id,product_unit_id,quantity,purchase_unit_price,discount_amount,net_before_tax,landed_cost_allocation,landed_unit_cost,tax_code_id,tax_amount,line_total) VALUES ($1,$2,$3,$4,8.000000,50.0000,0.0000,400.0000,0.0000,50.0000,NULL,0.0000,400.0000)`, [ids.line, ids.invoice, ids.variant, ids.productUnit]);
  await client.query(`INSERT INTO purchase_returns (id,branch_id,document_number,document_date,document_version,counterparty_id,warehouse_id,source_purchase_invoice_id,total,posted_at,created_by,deleted_at,deleted_by,delete_reason) VALUES ($1,$3,1,CURRENT_DATE,1,$4,$5,$6,150.0000,now(),$7,NULL,NULL,NULL),($2,$3,2,CURRENT_DATE,1,$4,$5,$6,50.0000,now(),$7,now(),$7,'reversed test return')`, [ids.activeReturn, ids.deletedReturn, ids.branch, ids.counterparty, ids.warehouse, ids.invoice, ids.user]);
  await client.query(`INSERT INTO purchase_return_lines (id,purchase_return_id,source_purchase_invoice_line_id,variant_id,quantity,commercial_unit_value_snapshot,inventory_unit_cost_snapshot,cost_variance,tax_amount,line_total) VALUES ('51000000-0000-4000-8000-000000000016',$1,$2,$3,3.000000,50.0000,48.0000,6.0000,0.0000,150.0000),('51000000-0000-4000-8000-000000000017',$4,$2,$3,1.000000,50.0000,48.0000,2.0000,0.0000,50.0000)`, [ids.activeReturn, ids.line, ids.variant, ids.deletedReturn]);
  return ids;
}
...
test("03.F Purchasing/Tax remains canonical after its 03.06 constraint slice", async (t) => {
  ...
  try {
    ...
    await withClient(async (client) => {
      ...
      assert.deepEqual(viewCols.rows.map((row) => [row.column_name,row.data_type]), [["source_purchase_invoice_line_id","uuid"],["purchased_quantity","numeric(18,6)"],["posted_returned_quantity","numeric(18,6)"],["returnable_quantity","numeric(18,6)"]]);
