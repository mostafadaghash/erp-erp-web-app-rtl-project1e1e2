...
const EXPECTED_COLUMNS = {
  ...
  counterparty_roles: [
    ["counterparty_id", "uuid", true],
    ["role", "text", true],
  ],
  customer_profiles: [
    ["counterparty_id", "uuid", true],
    ["default_price_list_id", "uuid", false],
    ["credit_limit", "numeric(18,4)", false],
  ],
  supplier_profiles: [
    ["counterparty_id", "uuid", true],
    ["notes", "text", false],
...
test("03.B creates only the canonical Counterparties schema shape", async (t) => {
  ...
  try {
    ...
    await withClient(async (client) => {
      ...
      assert.equal(tableResult.rowCount, COUNTERPARTY_TABLES.length);
      assert.deepEqual(
        tableResult.rows.map((row) => row.tablename),
        [...COUNTERPARTY_TABLES].sort(),
      );
      ...
      const actualByTable = Object.fromEntries(
        COUNTERPARTY_TABLES.map((table) => [table, []]),
      );
