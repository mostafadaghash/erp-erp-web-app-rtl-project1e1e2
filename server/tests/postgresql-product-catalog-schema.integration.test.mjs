...
test("03.C Product Catalog schema remains canonical after later schema migrations", async (t) => {
      ...
      const baseUnitAlias = await client.query(
        `SELECT
           EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema = 'public' AND table_name = 'product_units' AND column_name = 'is_base') AS has_is_base,
           EXISTS (SELECT 1 FROM information_schema.columns
                   WHERE table_schema = 'public' AND table_name = 'products' AND column_name = 'base_unit_id') AS has_base_unit_id`,
      );
