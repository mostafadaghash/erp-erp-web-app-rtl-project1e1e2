...
test("03.E creates the canonical Sales schema and returnable helper view", async (t) => {
  ...
  try {
    ...
    await withClient(async (client) => {
      const expectedTables = [...CORE_TABLES,...COUNTERPARTY_TABLES,...PRODUCT_TABLES,...INVENTORY_TABLES,...SALES_TABLES].sort();
      ...
      const view = await client.query(
        `SELECT c.relname, c.relkind FROM pg_catalog.pg_class c
         ...
         WHERE n.nspname='public' AND c.relname='sales_returnable_quantities_v'`,
      );
      ...
      const columns = await client.query(
        `SELECT c.relname AS table_name, a.attname AS column_name,
         ...
         ORDER BY c.relname,a.attnum`, [SALES_TABLES]);
      const actual = Object.fromEntries(SALES_TABLES.map((table) => [table, []]));
      ...
      const constraints = await client.query(
        `SELECT count(*)::int AS count FROM pg_catalog.pg_constraint con
         ...
         JOIN pg_catalog.pg_namespace n ON n.oid=c.relnamespace
         WHERE n.nspname='public' AND c.relname = ANY($1::text[])`, [SALES_TABLES]);
      ...
      assert.equal(futureDomain.rows[0].absent, true, "03.F Purchasing must remain absent");

      const ids = {
        line: "00000000-0000-4000-8000-000000000101",
        invoice: "00000000-0000-4000-8000-000000000102",
        variant: "00000000-0000-4000-8000-000000000103",
        unit: "00000000-0000-4000-8000-000000000104",
        activeReturn: "00000000-0000-4000-8000-000000000105",
        deletedReturn: "00000000-0000-4000-8000-000000000106",
        branch: "00000000-0000-4000-8000-000000000107",
        warehouse: "00000000-0000-4000-8000-000000000108",
        user: "00000000-0000-4000-8000-000000000109",
      };
      await client.query(
        `INSERT INTO sales_invoice_lines
         (id,invoice_id,variant_id,product_unit_id,quantity,unit_price,price_source,discount_amount,tax_amount,line_total)
         VALUES ($1,$2,$3,$4,5.000000,100.0000,'PRICE_LIST',0.0000,0.0000,500.0000)`,
        [ids.line,ids.invoice,ids.variant,ids.unit],
      );
      await client.query(
        `INSERT INTO sales_returns
         (id,branch_id,document_number,document_date,document_version,counterparty_id,warehouse_id,source_invoice_id,subtotal,tax_total,grand_total,posted_at,created_by)
         VALUES ($1,$2,1,CURRENT_DATE,1,NULL,$3,$4,200.0000,0.0000,200.0000,now(),$5),
                ($6,$2,2,CURRENT_DATE,1,NULL,$3,$4,100.0000,0.0000,100.0000,now(),$5)`,
        [ids.activeReturn,ids.branch,ids.warehouse,ids.invoice,ids.user,ids.deletedReturn],
      );
      await client.query(
        "UPDATE sales_returns SET deleted_at=now(), deleted_by=$1, delete_reason='reversed test return' WHERE id=$2",
        [ids.user,ids.deletedReturn],
      );
      await client.query(
        `INSERT INTO sales_return_lines
         (id,sales_return_id,source_invoice_line_id,variant_id,product_unit_id,quantity,unit_price,discount_amount,historical_unit_cost,line_total)
         VALUES
         ('00000000-0000-4000-8000-000000000110',$1,$2,$3,$4,2.000000,100.0000,0.0000,60.0000,200.0000),
         ('00000000-0000-4000-8000-000000000111',$5,$2,$3,$4,1.000000,100.0000,0.0000,60.0000,100.0000)`,
        [ids.activeReturn,ids.line,ids.variant,ids.unit,ids.deletedReturn],
      );
      const returnable = await client.query(
        "SELECT sold_quantity,posted_returned_quantity,returnable_quantity FROM sales_returnable_quantities_v WHERE source_invoice_line_id=$1",
        [ids.line],
      );
      assert.deepEqual(returnable.rows[0], {
        sold_quantity: "5.000000",
        posted_returned_quantity: "2.000000",
        returnable_quantity: "3.000000",
      });
      ...
      assert.equal(history.rows[5].version, "0006");
      assert.equal(history.rows[5].name, "sales");
      assert.match(history.rows[5].checksum, /^[0-9a-f]{64}$/);
    });
