...
test("03.E Sales schema remains canonical after later schema migrations", async (t) => {
  ...
  try {
    ...
    await withClient(async (client) => {
      ...
      const futureDomain=await client.query("SELECT to_regclass('public.gl_accounts') IS NULL AS absent"); assert.equal(futureDomain.rows[0].absent,true,"03.H Accounting must remain absent");
      ...
      const history=await client.query("SELECT version,name,checksum FROM schema_migrations ORDER BY version"); assert.equal(history.rowCount,8); const target=history.rows.find((row)=>row.version==="0006"); assert.equal(target?.name,"sales"); assert.match(target?.checksum??"",/^[0-9a-f]{64}$/);
    });
