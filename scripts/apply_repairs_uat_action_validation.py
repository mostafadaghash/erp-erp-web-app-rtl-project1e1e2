...
path = Path("src/components/RepairsPage.tsx")
text = path.read_text()
...
validation_block = '''  const laborCostAmount = Number(form.laborCost || 0);
...
''' + validation_block + '''  useEffect(() => {''',
)

replace_once(
    "create handler guard",
    '''    if (saving) return;
    if (Number(form.deposit) > 0 && !accountId)''',
    '''    if (saving) return;
...
alert_class = "rounded-lg bg-amber-50 p-3 text-sm font-medium text-amber-800"
replace_once(
    ...
    f'''              {{collectionValidationReason && <p role="alert" className="{alert_class}">{{collectionValidationReason}}</p>}}
              <div className="flex gap-3">
                <button
                  className="btn-primary flex-1"
...
replace_once(
    ...
    f'''              {{refundValidationReason && <p role="alert" className="{alert_class}">{{refundValidationReason}}</p>}}
              <div className="flex gap-3">
                <button
                  className="btn-primary flex-1"
...
replace_once(
    ...
    f'''              {{transitionValidationReason && <p role="alert" className="{alert_class}">{{transitionValidationReason}}</p>}}
              <div className="flex gap-3">
                <button
                  className="btn-primary flex-1"
...
replace_once(
    ...
    f'''              {{editValidationReason && <p role="alert" className="sm:col-span-2 {alert_class}">{{editValidationReason}}</p>}}
              <div className="flex gap-3 sm:col-span-2">
...
replace_once(
    ...
    f'''              {{createValidationReason && <p role="alert" className="{alert_class}">{{createValidationReason}}</p>}}
              <div className="flex gap-3 pt-2">
