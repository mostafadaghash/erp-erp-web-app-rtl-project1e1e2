...
export const postOpeningBalance = mutation({ args: { accountId: v.id("financialAccounts"), amount: v.number(), date: v.string(), requestId: v.string() }, handler: async (ctx, args) => {
  ...
  if (await ctx.db.query("financialTransactions").withIndex("by_reference", q => q.eq("referenceType", "opening_account").eq("referenceId", String(account._id))).first()) throw new ConvexError("تم تسجيل رصيد افتتاحي لهذا الحساب");
  if (account.currentBalance !== 0) throw new ConvexError("الحساب لا يبدأ من صفر");
  if (args.amount === 0) throw new ConvexError("لا حاجة لحركة افتتاحية صفرية");
  return (await postFinancialTransaction(ctx, user, { type: "opening_balance", requestId: args.requestId, date: args.date, amount: args.amount,
    description: `رصيد افتتاحي: ${account.name}`, branchId: account.branchId, referenceType: "opening_account", referenceId: String(account._id), movements: [{ accountId: account._id, signedAmount: args.amount }] })).transactionId;
} });
...
export const confirmInitialization = mutation({ args: {}, handler: async (ctx) => { const user = await requirePermission(ctx, "initialize_finance"); const settings = await ctx.db.query("financeSettings").first(); if (!settings) throw new ConvexError("أدخل تاريخ بدء النظام أولاً"); if (settings.isInitialized) throw new ConvexError("تم تشغيل النظام المالي بالفعل");
  const branches = await ctx.db.query("branches").collect(); for (const branch of branches.filter(b => b.isActive)) { const accounts = await ctx.db.query("financialAccounts").withIndex("by_branch", q => q.eq("branchId", branch._id)).collect(); if (!accounts.some(a => a.isActive && a.type === "cash")) throw new ConvexError(`يجب إنشاء خزينة نقدية نشطة للفرع: ${branch.name}`); }
...
export const ledger = query({ args: { branchId: v.optional(v.id("branches")), accountId: v.optional(v.id("financialAccounts")), paginationOpts: paginationOptsValidator }, handler: async (ctx, args) => { const user = await requirePermission(ctx, "view_finance"); const branchId = user.role === "admin" || user.role === "accountant" ? args.branchId : user.branchId; if (!branchId) throw new ConvexError("اختر فرعاً"); if (args.accountId) { const account = await requireActiveFinancialAccount(ctx, args.accountId); assertFinancialAccountBranch(account, branchId); return await ctx.db.query("financialMovements").withIndex("by_account", q => q.eq("accountId", account._id)).order("desc").paginate(args.paginationOpts); } return await ctx.db.query("financialMovements").withIndex("by_branch_date", q => q.eq("branchId", branchId)).order("desc").paginate(args.paginationOpts); } });
