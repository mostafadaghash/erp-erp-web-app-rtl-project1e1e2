...
const applicationTables = {
  ...
  generalLedgerPeriodBalances: defineTable({key:v.string(),branchId:v.id("branches"),accountId:v.id("chartOfAccounts"),periodKey:v.string(),debitTotal:v.number(),creditTotal:v.number(),netDebitMovement:v.number(),updatedAt:v.number(),lastEntryId:v.id("journalEntries")}).index("by_key",["key"]).index("by_branch_period",["branchId","periodKey"]),
