...
const applicationTables = {
  generalLedgerSettings: defineTable({
    baseCurrency:v.literal("EGP"),chartVersion:v.string(),status:v.literal("foundation_ready"),operationalPostingEnabled:v.boolean(),cutoverDate:v.string(),initializedAt:v.number(),initializedBy:v.string(),initializationRequestId:v.string(),initializationFingerprint:v.string(),
  ...
  generalLedgerPeriodBalances: defineTable({key:v.string(),branchId:v.id("branches"),accountId:v.id("chartOfAccounts"),periodKey:v.string(),debitTotal:v.number(),creditTotal:v.number(),netDebitMovement:v.number(),updatedAt:v.number(),lastEntryId:v.id("journalEntries")}).index("by_key",["key"]).index("by_branch_period",["branchId","periodKey"]).index("by_period",["periodKey"]),
