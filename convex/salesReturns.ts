...
export const reverse = mutation({ args: { id: v.id("salesReturns"), reason: v.string(), date: v.string(), requestId: v.string() }, handler: async (ctx, args) => {
  ...
  const requestKey = `${user.userId}:${args.requestId.trim()}`; if (!args.requestId.trim()) throw new ConvexError("معرف طلب الإلغاء مطلوب");
  if (note.status === "reversed") { if (note.reversalRequestId === requestKey) return note._id; throw new ConvexError("تم إلغاء الإشعار الدائن بالفعل بطلب مختلف"); }
  if (!args.reason.trim()) throw new ConvexError("سبب الإلغاء مطلوب"); await requireFinanceInitialized(ctx, args.date);
  const invoice = await ctx.db.get(note.invoiceId); if (!invoice) throw new ConvexError("الفاتورة غير موجودة");
  for (const item of note.items) await changeProductStock(ctx, user, { productId: item.productId, quantityDelta: -item.quantityReturned, unitCost: item.historicalUnitCost, type: INVENTORY_MOVEMENT_TYPES.sale, reason: `إلغاء مرتجع ${note.creditNoteNumber}`, referenceId: String(note._id), referenceType: "sales_return_reversal" });
