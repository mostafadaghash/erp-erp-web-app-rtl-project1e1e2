...
export const updateStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.status === "cancelled" && (!args.date || !args.requestId)) throw new ConvexError("تاريخ ومعرف طلب الإلغاء مطلوبان");
    ...
    if (args.status === "cancelled" && repair.customerId && args.date && args.requestId) await postCustomerLedgerEntry(ctx, user, { type: "repair_cancel", requestId: args.requestId, customerId: repair.customerId, branchId: repair.branchId!, date: args.date, receivableDelta: -repair.remaining, advanceDelta: 0, purchasesDelta: -repair.totalCost, description: `إلغاء الصيانة ${repair.repairNumber}`, referenceType: "repair", referenceId: String(repair._id), referenceNumber: repair.repairNumber });
