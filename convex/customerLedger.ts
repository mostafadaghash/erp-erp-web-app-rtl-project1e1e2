...
export const legacyReview = query({ args: { branchId: v.id("branches") }, handler: async (ctx, args) => {
  ...
  const customers = (await ctx.db.query("customers").collect()).filter(customer => customer.branchId === args.branchId);
