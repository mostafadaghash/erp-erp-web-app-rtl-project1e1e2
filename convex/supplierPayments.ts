...
export const reverse = mutation({ args: { paymentId: v.id("supplierPayments"), reason: v.string(), date: v.string(), requestId: v.string() }, handler: async (ctx, args) => {
  ...
  if (!requestId || requestId.length > 200) throw new ConvexError("معرف طلب العكس غير صالح"); if (!reason) throw new ConvexError("سبب العكس مطلوب"); if (!/^\d{4}-\d{2}-\d{2}$/.test(args.date)) throw new ConvexError("تاريخ العكس غير صالح");
