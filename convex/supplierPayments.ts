...
function fingerprint(input: { supplierId: string; branchId: string; accountId: string; date: string; notes?: string; allocations: readonly { purchaseReceiptId: string; amount: number }[] }) {
  return JSON.stringify({ supplierId: input.supplierId, branchId: input.branchId, accountId: input.accountId, date: input.date, notes: input.notes?.trim() || undefined, allocations: canonicalAllocations(input.allocations) });
}
...
export const create = mutation({ args: { supplierId: v.id("suppliers"), branchId: v.optional(v.id("branches")), accountId: v.id("financialAccounts"), date: v.string(), requestId: v.string(), notes: v.optional(v.string()), allocations: v.array(allocationValidator) }, handler: async (ctx, args) => {
  ...
  if (args.allocations.some(x => !Number.isFinite(x.amount) || x.amount <= 0 || !hasAtMostTwoDecimals(x.amount))) throw new ConvexError("كل مبلغ توزيع يجب أن يكون موجباً ويمثل عدداً صحيحاً من القروش دون تقريب");
  const sorted = canonicalAllocations(args.allocations), requestFingerprint = fingerprint({ branchId: String(branchId), supplierId: String(args.supplierId), accountId: String(args.accountId), date: args.date, notes: args.notes, allocations: sorted });
...
export const reverse = mutation({ args: { paymentId: v.id("supplierPayments"), reason: v.string(), date: v.string(), requestId: v.string() }, handler: async (ctx, args) => {
  const user = await requirePermission(ctx, "reverse_supplier_payments"); if (!user.permissions.includes("reverse_financial_transactions")) throw new ConvexError("إلغاء دفعة المورد يتطلب صلاحية إلغاء المعاملات المالية");
  ...
  const reason = args.reason.trim(), requestId = args.requestId.trim();
  if (!requestId || requestId.length > 200) throw new ConvexError("معرف طلب الإلغاء غير صالح"); if (!reason) throw new ConvexError("سبب الإلغاء مطلوب"); if (!/^\d{4}-\d{2}-\d{2}$/.test(args.date)) throw new ConvexError("تاريخ الإلغاء غير صالح");
  const reversalFingerprint = JSON.stringify({ requestId, date: args.date, reason });
  if (payment.status === "reversed") { if (payment.reversalRequestId !== requestId) throw new ConvexError("تم إلغاء سند الدفع سابقاً بطلب إلغاء مختلف"); if (payment.reversalFingerprint !== reversalFingerprint) throw new ConvexError("أعيد استخدام معرف طلب الإلغاء بتاريخ أو سبب مختلف"); return payment.reversalFinancialTransactionId; }
