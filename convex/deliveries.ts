...
export const updateStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (delivery.orderId && (args.status === "shipped" || args.status === "returned")) {
      const order = await ctx.db.get(delivery.orderId);
      if (!order) throw new ConvexError("أمر البيع المرتبط بالشحنة غير موجود");
      const nextOrderStatus = args.status === "shipped" ? "handed_to_shipping" : "ready";
      await ctx.db.patch(order._id, { status: nextOrderStatus });
      await applyOrderStatsChange(ctx, order, { ...order, status: nextOrderStatus });
      await logAction(ctx, user, {
        action: "update_status",
        module: "orders",
        recordId: String(order._id),
        recordLabel: order.orderNumber,
        details: args.status === "shipped"
          ? `تم تسليم أمر البيع ${order.orderNumber} لشركة الشحن عبر ${delivery.deliveryNumber}`
          : `أعيد أمر البيع ${order.orderNumber} إلى تم التجهيز بعد إرجاع ${delivery.deliveryNumber}`,
        ...
        relatedNumber: delivery.deliveryNumber,
        before: { status: order.status },
        after: { status: nextOrderStatus },
      });
    }
...
export const reverseConfirmation=mutation({args:{deliveryId:v.id("deliveries"),reason:v.string(),date:v.string(),requestId:v.string()},handler:async(ctx,args)=>{
 const user=await requirePermission(ctx,"reverse_cod_collections"),reason=normalize(args.reason,"سبب الإلغاء"),requestId=normalize(args.requestId,"معرف الطلب"),fp=fingerprint({delivery:String(args.deliveryId),reason,date:args.date});const d=await ctx.db.get(args.deliveryId);if(!d||d.status!=="delivered"||!d.currentConfirmationId||!d.invoiceId||!d.orderId||!d.customerId||!d.branchId)throw new ConvexError("التوصيل غير مؤهل للعكس");const confirmation=await ctx.db.get(d.currentConfirmationId);if(!confirmation||confirmation.status!=="posted")throw new ConvexError("تأكيد التسليم الحالي غير صالح");if(confirmation.reversalRequestId===requestId){if(confirmation.reversalFingerprint!==fp)throw new ConvexError("أعيد استخدام requestId ببيانات مختلفة");return confirmation.reversalFinancialTransactionId??confirmation._id;}if(confirmation.reversalRequestId)throw new ConvexError("سبق الإلغاء بطلب مختلف");const [settled,invoice,returns]=await Promise.all([ctx.db.query("codSettlementItems").withIndex("by_confirmation",q=>q.eq("confirmationId",confirmation._id)).first(),ctx.db.get(d.invoiceId),ctx.db.query("salesReturns").withIndex("by_invoice",q=>q.eq("invoiceId",d.invoiceId!)).filter(q=>q.eq(q.field("status"),"posted")).first()]);if(settled||returns||!invoice)throw new ConvexError("آثار لاحقة تمنع الإلغاء");const cod=confirmation.codAmount;if(roundMoney(invoice.paid)!==roundMoney((d.prepaidAmount??0)+cod)||roundMoney(invoice.remaining)!==0)throw new ConvexError("تغيرت الفاتورة بعد التحصيل ولا يمكن الإلغاء");await requireFinanceInitialized(ctx,args.date);let financial:Id<"financialTransactions">|undefined,ledger:Id<"customerLedgerEntries">|undefined;if(cod>0){if(!confirmation.financialTransactionId)throw new ConvexError("معاملة COD مفقودة");financial=await reversePostedFinancialTransaction(ctx,user,{transactionId:confirmation.financialTransactionId,reason,date:args.date,requestId,referenceType:"delivery_confirmation",referenceId:String(confirmation._id),referenceNumber:d.deliveryNumber});const entry=await postCustomerLedgerEntry(ctx,user,{type:"delivery_cod_reversal",requestId:`${requestId}:ledger`,customerId:d.customerId,branchId:d.branchId,date:args.date,receivableDelta:cod,advanceDelta:0,purchasesDelta:0,description:`إلغاء تحصيل ${d.deliveryNumber}: ${reason}`,referenceType:"delivery_confirmation_reversal",referenceId:String(confirmation._id),referenceNumber:d.deliveryNumber,originalEntryId:confirmation.customerLedgerEntryId});ledger=entry.entryId;const paid=roundMoney(invoice.paid-cod),remaining=roundMoney(invoice.remaining+cod);await ctx.db.patch(invoice._id,{paid,remaining,status:deriveInvoiceStatus({netTotal:invoice.netTotal??invoice.total,creditedTotal:invoice.creditedTotal??0,paid,remaining})});}const order=await ctx.db.get(d.orderId);if(!order)throw new ConvexError("الطلب المرتبط بالتوصيل غير موجود");await ctx.db.patch(d.orderId,{status:"handed_to_shipping"});await applyOrderStatsChange(ctx,order,{...order,status:"handed_to_shipping"});await logAction(ctx,user,{action:"update_status",module:"orders",recordId:String(order._id),recordLabel:order.orderNumber,details:`تم عكس استلام ${order.orderNumber} وإعادته إلى شركة الشحن`,branchId:order.branchId,sourceType:"order",sourceId:String(order._id),sourceNumber:order.orderNumber,relatedType:"delivery",relatedId:String(d._id),relatedNumber:d.deliveryNumber,before:{status:order.status},after:{status:"handed_to_shipping"}});await ctx.db.patch(confirmation._id,{status:"reversed",reversedAt:Date.now(),reversedBy:user.userId,reversalReason:reason,reversalDate:args.date,reversalRequestId:requestId,reversalFingerprint:fp,reversalFinancialTransactionId:financial,reversalCustomerLedgerEntryId:ledger});await ctx.db.patch(d._id,{status:"shipped",deliveredDate:undefined,currentConfirmationId:undefined,reversedAt:Date.now(),reversedBy:user.userId,reversalReason:reason,reversalDate:args.date,reversalRequestId:requestId,reversalFingerprint:fp,reversalFinancialTransactionId:financial,reversalCustomerLedgerEntryId:ledger});await logAction(ctx,user,{action:"reverse",module:"deliveries",recordId:String(d._id),recordLabel:d.deliveryNumber,details:`إلغاء محاولة التأكيد ${confirmation.attemptNumber}: ${reason}`,branchId:d.branchId,sourceType:"delivery",sourceId:String(d._id),sourceNumber:d.deliveryNumber,relatedType:"delivery_confirmation",relatedId:String(confirmation._id),relatedNumber:String(confirmation.attemptNumber),financialTransactionId:financial?String(financial):undefined,reversalOfId:confirmation.financialTransactionId?String(confirmation.financialTransactionId):undefined,before:{status:"delivered",codAmount:cod},after:{status:"shipped",date:args.date,codAmount:cod,reversalReason:reason}});return financial??confirmation._id;
}});
