...
test("UI-05 keeps create requestId stable while retrying", () => assert.match(source, /createDelivery\([\s\S]*requestId:operationRequestId\.current/));
test("UI-06 keeps confirmation requestId stable while retrying", () => assert.match(source, /confirmDelivered\([\s\S]*requestId:operationRequestId\.current/));
test("UI-07 keeps settlement requestId stable while retrying", () => assert.match(source, /createSettlement\([\s\S]*requestId:operationRequestId\.current/));
test("UI-08 keeps reversal requestId stable while retrying", () => assert.match(source, /reverseConfirmation\([\s\S]*requestId:operationRequestId\.current/));
