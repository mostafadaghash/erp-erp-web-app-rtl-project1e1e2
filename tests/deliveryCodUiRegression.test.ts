...
test("UI-03 uses real paginated delivery query", () =>
  assert.match(source, /usePaginatedQuery\(api\.deliveries\.listPaginated/));
...
test("UI-09 create is protected by the shared busy guard", () =>
  assert.match(source, /const run\s*=\s*async[\s\S]*if \(busy\) return/));
...
test("UI-13 exposes a shipping modal", () =>
  assert.match(source, /ship:\s*"تسجيل إرسال الشحنة"/));
test("UI-14 exposes a delivery confirmation modal", () =>
  assert.match(source, /deliver:\s*"تسجيل التسليم والتحصيل"/));
test("UI-15 exposes cancellation and return modals", () => {
  assert.match(source, /return:\s*"تسجيل مرتجع الشحنة"/);
...
test("UI-16 settlement and reversal collect date and reason", () => {
  ...
  assert.match(source, /"reverse-settlement":\s*"إلغاء تسوية التحصيل"/);
  assert.match(source, /سبب إلغاء التسوية/);
  assert.match(source, /type="date"/);
});
test("UI-17 protects printing and awaits its DTO", () =>
  assert.match(source, /if \(busy \|\| !canPrint\) return;[\s\S]*const dto = await/));
...
test("UI-20 accountant settlement source is gated by settlement permission", () =>
  assert.match(source, /settlementSources = useQuery\([\s\S]*api\.deliveries\.accountPicker,[\s\S]*canSettle/));
test("UI-21 shipping settlement pickers are not gated by confirmation permission", () =>
  assert.doesNotMatch(source, /settlementSources = useQuery\([^\n]*canConfirm/));
...
test("UI-22 settlement id and reason use independent state", () => {
  ...
  assert.match(source, /const openSettlementReversal[\s\S]*resetForm\(\)[\s\S]*setSelectedSettlementId\(settlementId\)/);
});
...
test("UI-25 real Convex delivery print query is awaited", () =>
  assert.match(source, /const dto = await convex\.query\(api\.deliveries\.printDelivery/));
test("UI-26 real Convex settlement print query is awaited", () =>
  assert.match(source, /const dto = await convex\.query\(api\.deliveries\.printCodSettlement/));
test("UI-27 delivery and settlement print structured Arabic vouchers", () => {
  for (const label of ["سند شحن", "سند تسوية", "توقيع شركة الشحن", "توقيع المحاسب", "الإجمالي", "الصافي"]) {
    assert.ok(source.includes(label), `missing voucher field: ${label}`);
  }
  assert.match(source, /dto\.items\.map/);
  assert.match(source, /escapeHtml\(dto\.reversalReason\)/);
});
test("UI-28 stats cards use branch-scoped getStats", () => {
  assert.match(source, /api\.deliveries\.getStats, activeBranch \? \{ branchId: activeBranch \} : "skip"/);
  for (const label of ["قيد التحصيل لدى شركات الشحن", "تمت تسويته", "أُلغي تحصيله", "رسوم شركات الشحن"]) {
    assert.ok(source.includes(label));
  }
});
test("UI-29 avoids prompt global print hooks and unsafe escapes", () =>
  assert.doesNotMatch(source, new RegExp(["window\\.prompt", "__deliveryPrint", ["as", "any"].join(" "), ["@ts", "ignore"].join("-")].join("|"))));
...
test("UI-31 opening any operation resets shared COD form state", () => {
  assert.match(source, /const resetForm = \(\) => \{[\s\S]*setAccountId\(""\)[\s\S]*setDestinationId\(""\)[\s\S]*setChecked\(new Set<string>\(\)\)/);
  assert.match(source, /const open = \(kind: Modal, row\?: Selected\) => \{[\s\S]*resetForm\(\)/);
  assert.match(source, /const openSettlementReversal[\s\S]*resetForm\(\)/);
});
...
test("UI-35 central validation covers create delivery required inputs", () => {
  for (const message of ["اختر الفرع", "اختر أمر البيع", "اختر الفاتورة", "أدخل المدينة", "أدخل عنوان التسليم", "أدخل شركة الشحن", "أدخل رسوم شحن صحيحة"]) {
...
test("UI-36 COD confirmation requires a clearing account only for positive COD", () => {
  ...
  assert.ok(source.includes("اختر حساب وسيط التحصيل"));
});
test("UI-37 settlement validates accounts selection gross fee and date", () => {
  for (const message of [
    "اختر حساب مبالغ التحصيل",
    "اختر الخزينة أو البنك المستلم",
    "يجب أن يختلف حساب التحصيل عن الحساب المستلم",
    "اختر شحنة واحدة على الأقل",
    "إجمالي المبلغ المحدد يجب أن يكون أكبر من صفر",
    "لا يمكن أن تتجاوز الرسوم إجمالي المبالغ المحددة",
    "اختر تاريخ التسوية",
  ]) assert.ok(source.includes(message), `missing ${message}`);
});
...
test("UI-39 invalid actions show the blocking reason and cannot run", () => {
  ...
  assert.match(source, /if \(validationReason\) \{ toast\.error\(validationReason\); return; \}/);
  assert.match(source, /aria-describedby=\{validationReason \? "delivery-action-validation" : undefined\}/);
  assert.match(source, /title=\{validationReason \?\? undefined\}/);
});
test("UI-40 date input is shown only for operations that send a date", () =>
  assert.match(source, /\["create", "deliver", "reverse-confirmation", "settle", "reverse-settlement"\]\.includes\(modal\)/));
