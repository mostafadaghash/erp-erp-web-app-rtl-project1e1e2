...
test("guard requires server cursor indexed account pagination",()=>{assert.doesNotMatch(api,/Number\([^)]*cursor|\boffset\b|range\.slice\s*\(/);assert.match(api,/by_account_branch_date_number_line/);assert.match(api,/\.paginate\(args\.paginationOpts\)/);assert.match(schema,/by_account_branch_date_number_line/);assert.match(helper,/generalLedgerDailyBalances/)});
test("guard rejects immutable journal APIs and fake marker assertions",()=>{assert.doesNotMatch(api,/export const (?:updateJournal|deleteJournal|removeJournal)|db\.patch\([^)]*journalLines/);assert.doesNotMatch(integration,/assert\.(?:ok|equal)\(\s*["'`]marker/i)});
