...
test("order printing uses print_orders and permission-gated controls", () => { assert.match(orders, /usePermission\("print_orders"\)/); assert.match(orders, /\{canPrint && <button onClick=\{\(\) => setPrintOrder\(order\)\}/); assert.match(orders, /\{canPrint && printOrder && <PrintModal/); });
